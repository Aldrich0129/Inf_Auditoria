...
class SimpleField(BaseModel):
    ...
    nombre: str = Field(description="Etiqueta visible para el usuario")
    tipo: Literal["texto", "numero", "lista", "texto_largo", "fecha"] = Field(
        description="Tipo de control UI"
    )
    requerido: bool = Field(False, description="Si el campo es obligatorio")
...
class ConditionalVariable(BaseModel):
    ...
    tipo_control: Literal["radio", "checkbox", "select", "derivado"] = Field(
        description="Tipo de control UI"
    )
