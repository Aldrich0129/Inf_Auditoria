...
def load_simple_fields(config_dir: Path) -> List[SimpleField]:
    ...
    if not data or 'variables_simples' not in data:
        logger.warning(f"No se encontraron variables_simples en {filepath}")
        return []
    
    fields = []
    for field_data in data['variables_simples']:
        try:
...
def load_conditional_variables(config_dir: Path) -> List[ConditionalVariable]:
    ...
    if not data or 'variables_condicionales' not in data:
        logger.warning(f"No se encontraron variables_condicionales en {filepath}")
        return []
    
    variables = []
    for var_data in data['variables_condicionales']:
        try:
